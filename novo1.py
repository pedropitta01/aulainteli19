...
def render_arquitetura(titulo: str, arq: ArquiteturaDetalhe):
    ...
    # Monta vantagens e desvantagens
    vantagens_html = "".join(
        f'<div class="vantagem">✓ {v}</div>' for v in arq.vantagens
    )
    desvantagens_html = "".join(
        f'<div class="desvantagem">✗ {d}</div>' for d in arq.desvantagens
    )

    # Monta HTML final como string única antes de passar ao Streamlit
    html = (
        '<div class="arq-card">'
